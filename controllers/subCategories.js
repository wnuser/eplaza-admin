...
const createSubCategory = async (req, res) => {
  ...
  const uploadPath = path.join(__dirname,'../public/images/sub-category/' + fileName);
  ...
  const saveSubCategory = await subCategoryModel.create(data).then(function(data){

      let successResponse = {data, message:"Sub-category create successfully"}; 
...
const updateSubCategory = async (req, res) => {
  ...
  if(req.files){
      ...
      const filePath = path.join(__dirname,'../public/images/sub-category/' + getSubCategory.image);
  ...
  const updateCategory = await subCategoryModel.update(data, {
      where: {
        id: data.subCategoryId
      }
    })
    .then(function(data){
      ...
      let successResponse = {data, message:"Sub-category Update successfully"}; 
      console.log('success', data);
          return res.status(200).json(successResponse);
...
const deleteSubCategory = async (req, res) => {
  ...
  const deleteSubCategory = await subCategoryModel.destroy({
    ...
    .then(function(data){
      ...
      let successResponse = {data, message:"Sub-category Delete successfully"}; 
      console.log('success', data);
          return res.status(200).json(successResponse);
